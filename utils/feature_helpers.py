...
def calculate_features(data: pd.DataFrame) -> np.ndarray:
    ...
    try:
        ...
        features.append(talib.RSI(data['close']))
        features.append(talib.MACD(data['close'])[0])
        features.append(talib.BBANDS(data['close'])[0])
...
def calculate_regime_features(data: pd.DataFrame) -> np.ndarray:
    ...
    try:
        ...
        # Trend
        features.append(talib.ADX(data['high'], data['low'], data['close'])[-1])
        
        # Volume
        features.append(data['volume'].mean() / data['volume'].std())
        
        # Momentum
        features.append(talib.RSI(data['close'])[-1])
