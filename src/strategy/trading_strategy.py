...
from src.strategy.indicators import calculate_rsi, calculate_macd, calculate_bollinger_bands, calculate_ema
...
class TradingStrategy:
    ...
    def get_trading_signal(self, symbol: str, timeframe: str = None) -> Dict:
        ...
        try:
            ...
            # If timeframe is specified, only analyze that timeframe
            if timeframe:
                timeframes_to_analyze = [timeframe]
            else:
                timeframes_to_analyze = self.timeframes

            # Calculate indicators for all timeframes
            for tf in timeframes_to_analyze:
                if tf in self.data[symbol]:
                    df_with_indicators = self.calculate_indicators(symbol, tf)
                    if not df_with_indicators.empty:
                        self.data[symbol][tf] = df_with_indicators
    ...
    def _generate_base_signal(self, symbol: str, latest_data: Dict) -> Dict:
        ...
        try:
            # Get market regime
            market_regime = self.ai_module.get_market_regime(symbol)
            ...
            for timeframe, data in latest_data.items():
                # Get base indicators
                rsi = data['rsi']
                macd = data['macd']
                ...
                # Calculate timeframe-specific signal strength
                tf_signal_strength = 0.0
                tf_signal_type = None
                
                # Trend following signals
                if ema_short > ema_long:
                    tf_signal_strength += 0.3
                    tf_signal_type = 'buy'
                ...
                # Volume confirmation
                avg_volume = data['volume_ma']
                if volume > avg_volume * 1.5:
                    tf_signal_strength += 0.1
                
                # Add timeframe signal to overall signal
                if tf_signal_type == signal_type:
                    signal_strength += tf_signal_strength
                elif signal_type is None:
                    signal_type = tf_signal_type
                    signal_strength = tf_signal_strength
                else:
                    # Conflicting signals - reduce overall strength
                    signal_strength = max(0, signal_strength - tf_signal_strength)
                
                # Add timeframe-specific reasons
                signal_reasons.append(f"{timeframe}: {tf_signal_type} ({tf_signal_strength:.2f})")
            ...
            if volatility > 0.02:  # High volatility
                signal_strength *= 0.9  # Reduce signal strength in high volatility
            
            # Risk management checks
            if signal_strength >= self.config['strategy']['signal']['min_strength']:
                # Calculate position size based on risk
                account_balance = self.get_account_balance()
                risk_per_trade = account_balance * self.config['risk']['base_risk_per_trade']
                
                # Adjust position size based on volatility
                position_size = risk_per_trade / (atr * 2)  # Use 2x ATR for stop loss
                
                # Ensure position size is within limits
                max_position = account_balance * self.config['risk']['max_position_size']
                ...
                return {
                    ...
                    'stop_loss': atr * 2,
                    'take_profit': atr * 4,  # 2:1 risk-reward ratio
            ...
            return {'action': 'none', 'grade': 'F', 'reason': 'insufficient_signal_strength'}
    ...
    def generate_signals(self, data: pd.DataFrame, indicators: pd.DataFrame) -> pd.Series:
        ...
        if daily_profit is not None and daily_targets['min_profit'] <= daily_profit <= daily_targets['max_profit']:
            ...
            macd_sell = (indicators['macd'] < indicators['macd_signal']) & (indicators['macd_hist'] < 0)
            ...
            # EMA signalen
            ema_buy = (indicators['ema_short'] > indicators['ema_long'])
            ema_sell = (indicators['ema_short'] < indicators['ema_long'])
            
            # Volume signalen
            volume_buy = (data['volume'] > indicators['volume_ma'])
            ...
            buy_signals = (
                rsi_buy.astype(int) * 0.2 +
                macd_buy.astype(int) * 0.2 +
                bb_buy.astype(int) * 0.15 +
                ema_buy.astype(int) * 0.15 +
                volume_buy.astype(int) * 0.1 +
                stoch_buy.astype(int) * 0.2
            ) * strong_trend.astype(int)
            
            sell_signals = (
                rsi_sell.astype(int) * 0.2 +
                macd_sell.astype(int) * 0.2 +
                bb_sell.astype(int) * 0.15 +
                ema_sell.astype(int) * 0.15 +
                volume_sell.astype(int) * 0.1 +
                stoch_sell.astype(int) * 0.2
            ) * strong_trend.astype(int)
            
            # Genereer uiteindelijke signalen
            signals[buy_signals > 0.5] = 1
            signals[sell_signals > 0.5] = -1
